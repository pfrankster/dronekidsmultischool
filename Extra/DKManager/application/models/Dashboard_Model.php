...
class Dashboard_Model extends MY_Model {
    ...
    // pe Added Fix
    public function get_total_preenroll($school_id = null){
        
        $this->db->select('COUNT(G.id) AS total_preenroll');
        $this->db->from('preenrolls AS G');
        $this->db->where('G.status', 1);
        if($school_id){
             $this->db->where('G.school_id', $school_id);
        }
        return $this->db->get()->row()->total_preenroll;    
     }
