Main Update:
=========
 -Path: 	application/config/custom.php
 -Description:	Responsible for the rules of restriction, who can acesse what.
 Status: [Ok]
=========
 -Path:		application/modules/preenroll/controllers/Preenroll.php
 -Description:	Php class managin function fot the preenroll page
 Status: [Working on it]
=========
 -Path:		application/modules/preenroll/models/Preenroll_model.php
 -Description:	Php responsable for the comunication with the server.
 Status: [Working on it]
=========
 -Path:		application/modules/preenroll/views/index.php
 -Description:	View that will be showed when file asseced
 Status: [Working on it]
---------------------------------------------------
Language Update:
=========
 -Path:		application/language/english/sms_lang.php
 -Description:	Responsible for the words exemption in correct idiom.
 Status: [Ok]
=========
 -Path:		application/language/pt-br/sms_lang.php
 -Description:	New idiom for the system (creating)
 Status: [Working on it]
=========
 -Path:		application/core/MY_Controller.php
 -Description: it have reference to the language pack.
 Status: [Change Reference?]
=========
 -Path:		system/libraries/Session/Session.php
 -Description: it have reference to the language pack.
 Status: [Change Reference?]

---------------------------------------------------
Fixed:
=========
 -Path:		application/
 -Description:	
---------------------------------------------------

Maybe?:
=========
 -Path:		application/controllers/Dashboard.php
 -Description:	Have the functions used on the programe
=========
 -Path:		application/models/Dashboard_Model.php
 -Description:	What will being showed on dashboard?




---------------------------------------------------

 -Path:		application/
 -Description:	

